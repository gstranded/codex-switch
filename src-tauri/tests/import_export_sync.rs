...
requires_openai_auth = true
"#;
    codex_switch_lib::write_codex_live_atomic(&legacy_auth, Some(legacy_config))
...
"#
    ...
    let toml_text =
        fs::read_to_string(codex_switch_lib::get_codex_config_path()).expect("read config.toml");
...
fn sync_codex_provider_missing_auth_returns_error() {
    ...
    match err {
        codex_switch_lib::AppError::Config(msg) => {
    ...
    assert!(
        !codex_switch_lib::get_codex_config_path().exists(),
