...
/// 切换 Codex provider 后的自动历史同步：Codex 的历史列表会按当前
/// `model_provider` 精确过滤。这里把本机已知的其它历史桶迁到当前激活桶，
/// 让用户切换不同 Codex provider 后仍能看到完整会话列表。
///
/// 与既有迁移一样，改写前会备份 `.jsonl` 和 `state_5.sqlite`；失败不会写
/// completion marker，下一次切换可继续重试。
pub fn sync_all_codex_history_to_active_provider(
    db: &Database,
) -> Result<CodexHistoryProviderBucketMigrationOutcome, AppError> {
    let _op_guard = lock_codex_official_history_op();
    let codex_dir = get_codex_config_dir();
    let config_text = read_codex_config_text().unwrap_or_default();
    let target_provider_id = active_model_provider_id_from_config_text(&config_text);
    let mut source_provider_ids =
        collect_all_codex_history_model_provider_ids(db, &codex_dir, &config_text)?;
    source_provider_ids.remove(&target_provider_id);

    if source_provider_ids.is_empty() {
        return Ok(CodexHistoryProviderBucketMigrationOutcome {
            skipped_reason: Some("no_other_history_buckets".to_string()),
            ..Default::default()
        });
    }
    ...
    let migrated_state_rows = migrate_codex_state_dbs_to_provider(
        &codex_dir,
        &source_provider_ids,
        &target_provider_id,
        &backup_root,
    ...
    if migrated_jsonl_files == 0 && migrated_state_rows == 0 {
        return Ok(CodexHistoryProviderBucketMigrationOutcome {
            source_provider_ids: source_provider_ids.into_iter().collect(),
            skipped_reason: Some("nothing_to_sync".to_string()),
            ..Default::default()
        });
    }
...
fn active_model_provider_id_from_config_text(config_text: &str) -> String {
    config_text
        .parse::<DocumentMut>()
        .ok()
        .and_then(|doc| {
        ...
        .unwrap_or_else(|| OFFICIAL_OPENAI_CODEX_MODEL_PROVIDER_ID.to_string())
}

fn collect_all_codex_history_model_provider_ids(
    db: &Database,
    codex_dir: &Path,
    config_text: &str,
    ...
    // Codex's built-in official bucket is implicit when config.toml has no
    // `model_provider`, and older sessions may only carry it in state_5.sqlite.
    ids.insert(OFFICIAL_OPENAI_CODEX_MODEL_PROVIDER_ID.to_string());
    ids.insert(CC_SWITCH_CODEX_MODEL_PROVIDER_ID.to_string());
    for legacy in CC_SWITCH_LEGACY_CODEX_MODEL_PROVIDER_IDS {
        insert_model_provider_id(&mut ids, legacy);
    }

    collect_model_provider_ids_from_config_text(config_text, &mut ids);
    collect_model_provider_ids_from_stored_codex_providers(db, &mut ids)?;
    collect_model_provider_ids_from_codex_jsonl(codex_dir, &mut ids);
    collect_model_provider_ids_from_codex_state_dbs(codex_dir, config_text, &mut ids)?;

    Ok(ids)
}

fn collect_model_provider_ids_from_stored_codex_providers(
    db: &Database,
    ...
    for provider in providers.values() {
        if let Some(config_text) = provider
            ...
            .and_then(|value| value.as_str())
        {
            collect_model_provider_ids_from_config_text(config_text, ids);
        }
    }
    Ok(())
}

fn collect_model_provider_ids_from_config_text(config_text: &str, ids: &mut BTreeSet<String>) {
    let Ok(doc) = config_text.parse::<DocumentMut>() else {
        return;
    };
    if let Some(provider_id) = doc.get("model_provider").and_then(|item| item.as_str()) {
        insert_model_provider_id(ids, provider_id);
    }
    if let Some(model_providers) = doc.get("model_providers").and_then(|item| item.as_table()) {
        for provider_id in model_providers.iter().map(|(key, _)| key) {
            insert_model_provider_id(ids, provider_id);
        }
    }
}

fn collect_model_provider_ids_from_codex_jsonl(codex_dir: &Path, ids: &mut BTreeSet<String>) {
    let mut files = Vec::new();
    collect_jsonl_files(&codex_dir.join("sessions"), &mut files, 0, 8);
    collect_jsonl_files(&codex_dir.join("archived_sessions"), &mut files, 0, 4);
...
fn collect_model_provider_ids_from_codex_state_dbs(
    codex_dir: &Path,
    config_text: &str,
    ids: &mut BTreeSet<String>,
) -> Result<(), AppError> {
    for db_path in codex_state_db_paths(codex_dir, config_text) {
        ...
        .map_err(|e| AppError::Database(format!("打开 Codex state DB 失败: {e}")))?;
        conn.busy_timeout(Duration::from_secs(2)).map_err(|e| {
...
fn insert_model_provider_id(ids: &mut BTreeSet<String>, provider_id: &str) {
...
"#
    ...
    #[test]
    #[serial]
    fn syncs_all_codex_history_buckets_to_active_provider_after_switch() {
        let dir = tempdir().expect("tempdir");
        let _home_guard = EnvVarGuard::set("CC_SWITCH_TEST_HOME", dir.path());
        let _sqlite_guard = EnvVarGuard::set("CODEX_SQLITE_HOME", &dir.path().join("sqlite-home"));
        let codex_dir = dir.path().join(".codex");
        fs::create_dir_all(&codex_dir).expect("create codex dir");
        fs::write(
...
"#
        ...
        let session_dir = codex_dir.join("sessions/2026/07/07");
        fs::create_dir_all(&session_dir).expect("create session dir");
        let session_path = session_dir.join("switch-sync.jsonl");
        fs::write(
            &session_path,
            concat!(
                "{\"type\":\"session_meta\",\"payload\":{\"id\":\"s1\",\"model_provider\":\"openai\"}}\n",
                "{\"type\":\"session_meta\",\"payload\":{\"id\":\"s2\",\"model_provider\":\"deepseek\"}}\n",
                "{\"type\":\"session_meta\",\"payload\":{\"id\":\"s3\",\"model_provider\":\"custom\"}}\n",
                "{\"type\":\"session_meta\",\"payload\":{\"id\":\"s4\",\"model_provider\":\"openrouter\"}}\n",
                "{\"type\":\"response_item\",\"payload\":{\"text\":\"openai should stay in content\"}}\n",
            ),
        )
        .expect("write session");
        ...
        conn.execute_batch(
            ...
            INSERT INTO threads (id, model_provider) VALUES
                ('openai-thread', 'openai'),
                ('deepseek-thread', 'deepseek'),
                ('custom-thread', 'custom'),
                ('openrouter-thread', 'openrouter');",
        )
        .expect("seed state db");
        drop(conn);

        let outcome =
            sync_all_codex_history_to_active_provider(&db).expect("sync history after switch");
        assert_eq!(outcome.skipped_reason, None);
        assert_eq!(outcome.migrated_jsonl_files, 1);
        assert_eq!(outcome.migrated_state_rows, 3);
        ...
        assert!(session_text.contains("openai should stay in content"));

        let conn = Connection::open(&state_db_path).expect("reopen state db");
        let openrouter_count: i64 = conn
            .query_row(
                "SELECT COUNT(*) FROM threads WHERE model_provider = 'openrouter'",
                [],
                |row| row.get(0),
            )
            .expect("count openrouter rows");
        assert_eq!(openrouter_count, 4);
        drop(conn);

        let backup_parent = dir
            .path()
