...
impl ProviderService {
    ...
    fn sync_codex_history_after_switch(state: &AppState, result: &mut SwitchResult) {
        match crate::codex_history_migration::sync_all_codex_history_to_active_provider(
            state.db.as_ref(),
        ) {
            Ok(outcome) => {
                if let Some(reason) = outcome.skipped_reason {
                    log::debug!("○ Codex history auto-sync skipped after switch: {reason}");
                } else {
                    log::info!(
                        "✓ Codex history auto-sync after switch completed: sources={}, jsonl_files={}, state_rows={}",
            ...
            Err(err) => {
                log::warn!("✗ Codex history auto-sync after switch failed: {err}");
                result
                    .warnings
                    .push(format!("codex_history_auto_sync_failed:{err}"));
            }
        }
    }
    ...
    pub fn switch(state: &AppState, app_type: AppType, id: &str) -> Result<SwitchResult, AppError> {
        ...
        if should_hot_switch {
            ...
            if matches!(app_type, AppType::Codex) {
                Self::sync_codex_history_after_switch(state, &mut result);
            }
            return Ok(result);
        }
        ...
        let mut result = Self::switch_normal(state, app_type, id, &providers)?;
        if is_codex {
            Self::sync_codex_history_after_switch(state, &mut result);
        }
        Ok(result)
