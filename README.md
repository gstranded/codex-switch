...
- `Codex-Switch-0.4.0-Windows-x64-Setup.exe`：Windows 常规安装包，推荐大多数 Windows 用户使用。
- `Codex-Switch-0.4.0-Windows-x64.msi`：Windows MSI 安装包，适合偏好 MSI 或企业部署的场景。
- `Codex-Switch-0.4.0-Windows-x64-Portable.zip`：Windows 便携版，解压后运行 `codex-switch.exe`。
- `Codex-Switch-0.4.0-macOS-universal.dmg`：macOS 安装包，同时支持 Apple Silicon 和 Intel Mac。
- `Codex-Switch-0.4.0-macOS-universal.zip`：macOS `.app` 压缩包，解压后运行。
- `Codex-Switch-0.4.0-Linux-x64.AppImage`：Linux 便携版，赋予执行权限后运行。
- `Codex-Switch-0.4.0-Linux-x64.deb`：Debian / Ubuntu 安装包。
- `Codex-Switch-0.4.0-Linux-x64.rpm`：Fedora / RHEL / openSUSE 安装包。
