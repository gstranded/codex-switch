...
- `Codex-Switch-0.3.0-Windows-x64-Setup.exe`：Windows 常规安装包，推荐大多数 Windows 用户使用。
- `Codex-Switch-0.3.0-Windows-x64.msi`：Windows MSI 安装包，适合偏好 MSI 或企业部署的场景。
- `Codex-Switch-0.3.0-Windows-x64-Portable.zip`：Windows 便携版，解压后运行 `codex-switch.exe`。
- `Codex-Switch-0.3.0-macOS-universal.dmg`：macOS 安装包，同时支持 Apple Silicon 和 Intel Mac。
- `Codex-Switch-0.3.0-macOS-universal.zip`：macOS `.app` 压缩包，解压后运行。
- `Codex-Switch-0.3.0-Linux-x64.AppImage`：Linux 便携版，赋予执行权限后运行。
- `Codex-Switch-0.3.0-Linux-x64.deb`：Debian / Ubuntu 安装包。
- `Codex-Switch-0.3.0-Linux-x64.rpm`：Fedora / RHEL / openSUSE 安装包。
...
- **跨电脑聊天迁移**：可导出和导入 Codex 聊天记录；导入记录会安全合并并同步到当前供应商。
...
在 **设置 -> 数据 -> Codex 聊天记录** 中，可在原电脑导出 `.zip` 归档，再在新电脑导入。归档只包含 Codex 会话 JSONL、会话标题和线程索引，不会包含 API Key、供应商配置或登录凭据。

导入时会保留本机已有会话、跳过重复会话，并立即把新会话同步到当前供应商。之后再次切换供应商时，这些聊天记录仍会走同一套自动同步逻辑。
